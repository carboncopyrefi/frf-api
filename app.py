...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    evaluations_collection.create_index(
        [("evaluator", 1), ("submission_id", 1)],
        unique=True,
    )
...
def current_user(token: Optional[str] = Depends(security)) -> str:
    payload = _verify_token(token.credentials if token else None)
    if not payload:
        raise HTTPException(
...
def require_evaluator(address: str = Depends(current_user)) -> str:
    role = _role(address)
    if role != "evaluator":
        raise HTTPException(
...
def create_evaluation_with_answers( request: Request, evaluation_data: EvaluationCreate = Body(...), evaluator: str = Depends(require_evaluator)):
    ...
    existing = evaluations_collection.find_one({
        "evaluator": evaluator,
...
async def nonce():
    ...
    return NonceResp(nonce=_new_nonce())
...
async def verify(body: VerifyReq):
    ...
    if nonces[message.nonce] < _now():
        raise HTTPException(status_code=400, detail="Nonce expired")
    nonces.pop(message.nonce, None)  # single-use

    # checksum & role
    address = to_checksum_address(message.address)
    role = _role(address)
    token = _create_token(address, role)

    return VerifyResp(token=token, role=role)


@app.get("/session", response_model=SessionResp)
async def session(
    cred: Optional[HTTPAuthorizationCredentials] = Depends(security),
):
    """Return current session info from JWT."""
    payload = _verify_token(cred.credentials if cred else None)
    if not payload:
        raise HTTPException(status_code=204)  # No content
    return SessionResp(address=payload["sub"], chainId=42220)  # Celo


@app.post("/signout")
async def signout():
    """Client-side sign-out (just discard token)."""
    return {}  # empty JSON
